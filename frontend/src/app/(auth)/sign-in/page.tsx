...
export default function SignInPage() {
  ...
  const { signIn, isLoaded, fetchStatus } = signInResult as any;
  const isAuthReady = isLoaded || (fetchStatus !== 'loading' && fetchStatus !== 'fetching' && !!signInResult);
  const [loadingProvider, setLoadingProvider] = React.useState<string | null>(null);

  useEffect(() => {
    if (isAuthReady) {
      console.log("SignInPage: Auth is ready");
    }
  }, [isAuthReady]);
    ...
    >
      ...
      <div className="flex flex-col gap-3 mb-8">
        <button 
          ...
          disabled={!isAuthReady || loadingProvider !== null}
          type="button" 
        ...
        >
          {loadingProvider === 'oauth_google' ? (
            <svg className="w-5 h-5 mr-3 animate-spin text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ) : (
          ...
          {loadingProvider === 'oauth_google' ? "Connecting to Google..." : isAuthReady ? "Continue with Google" : "Loading auth..."}
        </button>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            {loadingProvider === 'oauth_facebook' ? (
              <svg className="w-5 h-5 mr-2 animate-spin text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            ) : (
              <Facebook className="w-5 h-5 mr-2 text-[#1877F2]" fill="#1877F2" stroke="none" />
            )}
            {loadingProvider === 'oauth_facebook' ? "Connecting..." : "Facebook"}
          </button>
          <button 
          ...
          >
            ...
            {loadingProvider === 'oauth_apple' ? "Connecting..." : "Apple"}
          </button>
        </div>
      </div>
